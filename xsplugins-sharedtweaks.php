 ...
 */
add_filter( 'gettext', 'translate_word_replacements', 100 );
add_filter( 'ngettext', 'translate_word_replacements', 100 );
 ...
 */
add_action( 'wp_roles_init', 'rename_wordpress_roles', 100 );
function rename_wordpress_roles( \WP_Roles $roles ) {
 ...
 */
add_action( 'wp_roles_init', 'remove_role_permissions', 100 );
function remove_role_permissions( \WP_Roles $roles ) {
	// Defines a list of permissions to remove.
	$remove_permissions = array();

	// Stop if we don't have any permissions to remove.
	if ( count( $remove_permissions ) < 1 ) {
		return;
	}
